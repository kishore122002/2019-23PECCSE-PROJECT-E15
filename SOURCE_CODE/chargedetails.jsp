...
<th scope="col">Reject</th>
 ...
 <%String paddresss=session.getAttribute("email").toString(); %>
       ...
       con=dbcon.create();
       PreparedStatement ps=con.prepareStatement("SELECT * FROM `transport`.`clientform` where charge!=' ' ");
 ...
 <td style="text-align: center;"><%=  rs.getString(11) %></td>
  <td style="text-align: center;"><%=  rs.getString(12) %></td>
     <td style="text-align: center;"><%=  rs.getString(13) %></td>
      <td><a href="accept.jsp?id=<%=rs.getString(1)%>&&weight=<%=rs.getString(11)%>"><button class="btn btn-primary">Accept</button></a></td>
      <td><a href="reject.jsp?id=<%=rs.getString(1)%>&&weight=<%=rs.getString(11)%>"><button class="btn btn-primary">Reject</button></a></td>
     </tr>
	<%} %>
